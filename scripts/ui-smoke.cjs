...
(async () => {
 ...
 await page.locator('#topbar-title').click();await page.getByRole('button',{name:'Notifications',exact:true}).click();
 ...
 assert.equal(await page.evaluate(()=>document.body.dataset.mode),'todos','notifications back did not return to tasks');
 assert.equal(await page.locator('#section-tasks').isVisible(),true,'notifications back did not restore the task list');
